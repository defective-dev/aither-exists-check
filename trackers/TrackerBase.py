...
class PTracker(Protocol):
    def setup_log_files(self, app_configs: AppConfig) -> None:
        return
    ...
    def setup_log_files(self, app_configs: AppConfig):
        ...
        if output_path is not None:
            log_path = os.path.join(
                os.path.expanduser(output_path), self.__class__.__name__
            )
        else:
        ...
        if app_configs.radarr.get("enabled"):
            out_category = os.path.join(
                log_path, app_configs.log_files["not_found_radarr"]
            )
            out_trump = os.path.join(log_path, app_configs.log_files["trump_radarr"])
            self.radarr_not_found_file = open(
                out_category, "w", encoding="utf-8", buffering=1
            )
            self.radarr_trump_file = open(
                out_trump, "w", newline="", encoding="utf-8", buffering=1
            )
            self.radarr_trump_writer = csv.DictWriter(
                self.radarr_trump_file,
        ...
        if app_configs.sonarr.get("enabled"):
            out_category = os.path.join(
                log_path, app_configs.log_files["not_found_sonarr"]
            )
            out_trump = os.path.join(log_path, app_configs.log_files["trump_sonarr"])
            self.sonarr_not_found_file = open(
                out_category, "w", encoding="utf-8", buffering=1
            )
            self.sonarr_trump_file = open(
                out_trump, "w", newline="", encoding="utf-8", buffering=1
            )
            self.sonarr_trump_writer = csv.DictWriter(
                self.sonarr_trump_file,
                fieldnames=csv_headers,
                delimiter=",",
                quotechar='"',
                quoting=csv.QUOTE_MINIMAL,
